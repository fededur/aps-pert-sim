...
author: 
  ...
  email: federico.duranovich@mpi.govt.nz
output: 
...
# Method
The effect of the APS perturbation routine was quantified by simulating the land area of individual farms.<br/>
To determine the effect of perturbation on farm land area estimates for data aggregated at varying cell sizes, the simulation involved generating a farm land area for 50,000 farms representing the population of NZ farms. The dataset was simulated from a Gamma distribution that assumed an average farm land area of 275 ha and a standard deviation of 46.75 ha. Each farm in the dataset was assigned a uniformly distributed random number between 0 and 1.
Land area data was perturbed using the APS perturbation routine (see code for routine details). Four sets of samples comprising 10, 100, 1,000 and 10,000 farms were randomly extracted from the dataset. Each set was used to represent different cell sizes with smaller sizes being similar to Territorial Authority cell sizes and larger sizes to Regional cell sizes. The sampling process was replicated 1,000 times using the replacement technique. This was done to calculate the mean percentage error (MPE) of each cell size set, which was used as a metric of (in)accuracy. The MPE represents the average deviation between simulated and perturbed land area values and can be seen as a metric of the noise added by the perturbation routine to the different cell sizes. MPE was calculated as:

$$MPE = \frac{1}{n} \sum \left( \frac{\text{ actual } - \text{ perturbed }}{\text{actual}}\right) \times 100$$

where $actual$ is the simulated land area value of each farm, $perturbed$ is the perturbed land area value of each farm and $n$ is the total number of farms in the cell size (*i.e.* 10, 100. 1,000 or 10,000). <br/>
...
set.seed(1234)
farm_data_actual <- simFarms(50000, shape_parameter = shape, scale_parameter = scale)

farm_data <- tibble(
  actual = farm_data_actual %>% pull(land_area),
  perturbed = perturbAg(farm_data_actual) %>% pull(land_area))

# estimate mpe in cells of 10, 100, 1000 or 10000 farms by sampling 1000 farms in each case
...
# estimate mpe and mape for a set of 1000 farms of land areas varying from 1 to 1000 ha
rn <- runif(n = 1000, min = 0, max = 1)

lnd_n <- crossing(
  rn = rn,
  land_area = seq(1, 1000, by = 0.5))

lnd_i <- crossing(
  rn = rn,
  land_area = seq(1, 1000, by = 1))
...
```{r Tab1, label="Tab1", warning=FALSE}
map(n, ~ farm_data %>%
                 sample_n(.x) %>%
                 mutate(n = .x)
               ) %>%
  ...
  kable(caption = "Total land area by cell size",
...
* **Aggregation at different cell sizes**. The error associated with low cells sizes is high compared with high cell sizes. In this exercise, it is shown that the error of aggregations including 10 farms can be $\pm$ 10 percent. Consider increase cell size to reduce error^[Note that the cell size chosen should also consider the error associated with sampling defined by the survey design (APS surveys are designed to produce satisfactory results at regional level) and other confidentiality rules (minimum of 3 farms per cell)]
